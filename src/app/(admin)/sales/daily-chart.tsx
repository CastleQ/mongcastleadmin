/** 이달 누적 매출 꺾은선 (일 단위). 거래 있는 날만 점·날짜, 위치는 달력 위치 그대로 */
export function DailyChart({ points, days, elapsed, target }: {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img" aria-label="이달 누적 매출 꺾은선 그래프">
      {ticks.map((v) => (
        <g key={v}>
      ...
      {points.map((p, i) => (
        <g key={p.day}>
          <circle cx={x(p.day)} cy={y(p.cum)} r="4" fill="#18181b" stroke="#fff" strokeWidth="2"><title>{`${p.day}일 누적 매출 ${won(p.cum)}원`}</title></circle>
